...
def compute_label_bins(y: Tensor, n_bins: int):
    while True:
        ...
        try: 
            rtdl_num_embeddings._check_bins(bins) # _check_bins takes list[tensor]
            break
        except:
            n_bins -= 1
            print(f"Due to duplicated bins, decrease n_bins by 1 and use {n_bins}")
        ...
        *,
        ...
        if n_classes is None:
            ...
            if label_bins is None:
                ...
                print("Init label_encoder as PiecewiseLinearEmbeddings")
                self.label_encoder = rtdl_num_embeddings.PiecewiseLinearEmbeddings(
                    bins=label_bins,
                    d_embedding=d_main,
                    activation=False,
                )
        ...
        self.ple_label_embedding = False if label_bins is None else True
        ...
        *,
        ...
        if self.training:
            ...
            if is_train:
                ...
                candidate_y = torch.cat([y_encoded, candidate_y])
        ...
        context = torch.mm(weights, candidate_y) # adding label context
        query = query + context
        ...
        if self.training:
            self._dequeue_and_enqueue(key_x, y_encoded) # enqueue current batch
        if query.ndim == 2:
...
def main(
    ...
    *,
    ...
    label_bin_edges = None
    if 'label_bins' in config:
        if dataset.task.is_regression:
            print(config['label_bins'])
            label_bin_edges = compute_label_bins(dataset.data['y']['train'], n_bins=config['label_bins']['n_bins'])
            print(label_bin_edges)
        else:
            print("Skip computing labels bins since current task is classification.")
    ...
    def freeze_key():
        ...
        for idx in torch.arange(dataset.size("train"), device=device).split(batch_size):
            ...
            key, y_encoded = model.calculate_key(x_num, x_cat, y)
            candidate_k.append(key)
            candidate_y.append(y_encoded)
